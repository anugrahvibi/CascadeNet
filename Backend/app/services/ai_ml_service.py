...
from src.models.lstm_predictor_wayanad import LSTMFloodPredictor, ZONE_FLOOD_THRESHOLDS
...
class AIMLService:
    def __init__(self):
        ...
        self._lstm = None
        self._router = None
        self._distress_signal = None

    def trigger_distress(self, zone_id: str):
        self._distress_signal = {
            "zone_id": zone_id,
            "timestamp": datetime.now().isoformat()
        }
        return self._distress_signal

    def get_distress(self):
        return self._distress_signal
    ...
    def lstm(self):
        if not self._lstm:
            ...
            # Fast-path for demo: Avoid heavy training on every restart
            print("[AIML] Using fast-path initialization for Hackathon demo...")
            self._lstm._trained = True 
            self._lstm._current_conditions = {
                zid: {"rainfall_mmhr": 2.5, "river_level_m": 1.0, "reservoir_outflow_cumecs": 1500}
                for zid in ZONE_FLOOD_THRESHOLDS
            }
            # Still call build/train but with KERAS_AVAILABLE=False to trigger instant fallback
            # Or just rely on the manual override above
        return self._lstm
