...
async def get_alerts_summary(scenario: str = '2024_peak'):
    ...
    # Check for active distress signal
    distress = ai_ml_service.get_distress()
    if distress:
        all_actions.append({
            "department": "ndrf_rescue",
            "alert_level": "RED",
            "action": f"!!! EMERGENCY: DISTRESS SIGNAL DETECTED IN {distress['zone_id']} !!! EXECUTE PROTOCOL S-10. SEARCH AND RESCUE DEPLOYMENT IMMEDIATE.",
            "time_window_hours": 0.0,
            "source": "FIELD_SENSOR_SOS",
            "priority": "CRITICAL",
            "is_distress": True,
            "timestamp": distress.get("timestamp")
        })
...
@router.post("/alerts/distress")
async def trigger_distress(
    background_tasks: BackgroundTasks, 
    zone_id: str = "ZONE_KALPETTA",
    phone_number: str = "9074121510"
):
    """Manually trigger a demo distress signal."""
    result = ai_ml_service.trigger_distress(zone_id)
    
    # Broadcast SMS to the specific phone number if provided, else default team
    target = phone_number
    background_tasks.add_task(
        dispatch_tactical_sms, 
        target, 
        f"!!! EMERGENCY: DISTRESS SIGNAL DETECTED IN {zone_id} !!! EXECUTE PROTOCOL S-10. SEARCH AND RESCUE DEPLOYMENT IMMEDIATE."
    )
    
    return result

@router.delete("/alerts/distress")
async def clear_distress(
    background_tasks: BackgroundTasks,
    phone_number: str = "9074121510"
):
    """Clear active distress signal."""
    result = ai_ml_service.clear_distress()
    
    # Send clear message to the responding terminal
    background_tasks.add_task(
        dispatch_tactical_sms,
        phone_number,
        "SOS Signal Terminated. Stand down field units."
    )
